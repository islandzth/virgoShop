...
class imageLib_Common
{
    ...
    private static $_imagickObj;
    private static $_imagick;
    ...
    function __construct($fileName)
    {
        ...
        $imagick = $this->testImagickInstalled();

        if ($imagick) {
            self::$_imagick = true;
            self::$_imagickObj = new Imagick();
            $this->image = self::$_imagickObj->readimage($fileName);
            self::$_imagickObj->setImageCompression(Imagick::COMPRESSION_JPEG);
            self::$_imagickObj->setImageCompressionQuality(100);
        } else {
    ...
    public function resizeImage($newWidth, $newHeight, $option = 0, $sharpen = false)
    {
        ...
        if (self::$_imagick) {
            if ($option == 4 || $option == 'crop') {
                self::$_imagickObj->cropThumbnailImage($newWidth, $newHeight);
            } else {
                self::$_imagickObj->resizeimage($newWidth, $newHeight, Imagick::FILTER_POINT, 1);
            }
        } else {
    ...
    {
        if (self::$_imagick) {
            self::$_imagickObj->writeimage($savePath);
        } else {
    ...
    public function testImagickInstalled()
    {
        if (extension_loaded("Imagick")) {
            $imagickInstalled = true;
        } else {
            $imagickInstalled = false;
        }
        return $imagickInstalled;
    }
