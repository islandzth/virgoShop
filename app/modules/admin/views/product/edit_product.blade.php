...
@section('content')
    <div class="row">
        <div class="col-md-6">
            ...
            @endif
            {{ Form::open(array('url' => URL::route('editProduct', array($productObj->product_id)),'id'=>'createCat','class'=>'form-horizontal form-vertical no-padding no-margin')) }}
                <h4>Sửa thông tin sản phẩm</h4>
                <div class="form-group">
                    <label for="inputEmail3" class="col-sm-4 col-md-4 control-label">Tên sản phẩm:</label>
                    <div class="col-sm-8 col-md-8">
                      <input type="text" name="nameProduct" class="form-control" id="inputEmail3" value="{{$productObj->name}}">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-8 col-md-8">
                      <select class="form-control" name="catProduct">
                        @foreach($listCat as $catObj)
                            @if($productObj->category == $catObj->category_id)
                  ...
                  @foreach($ProductOption as $optionObj)
                  <div class="form-group">
                    <label for="inputEmail3" class="col-sm-4 col-md-4 control-label">Màu sắc:</label>
                    <div class="col-sm-3 col-md-3">
                      <input type="text" name="colorProduct[]" class="form-control" id="inputEmail3" value="{{$optionObj->size}}" disabled="true">
                    </div>
                    <label for="inputEmail3" class="col-sm-1 col-md-1 control-label">Giá:</label>
                    <div class="col-sm-3 col-md-3">
                      <input type="text" name="priceProduct[]" class="form-control" id="inputEmail3" value="{{$optionObj->gia}}" disabled="true">
                    </div>
                  </div>
                  @endforeach
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8 col-md-8">
                      <input type="text" name="keyWordProduct" class="form-control" id="inputEmail3" value="{{$productObj->meta_keyword}}" >
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="inputPassword3" class="col-sm-4 col-md-4 control-label">Mô tả:</label>
                    <div class="col-sm-8 col-md-8">
                        <textarea class="form-control" name="descriptionProduct" rows="3" placeholder="hiển thị trong meta description (150 ki tu)">{{$productObj->description}}</textarea>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-8 col-md-8">
                      <div id="container_image">
                        ...
                        <div class="row" id="show_image" style="padding:10px 0px">
                            @foreach($productImage as $imageObj)
                            <div class="col-sm-12 col-md-12" id="show_detail_img" style="border: 1px solid #ebebeb;margin-bottom: 10px;">
                                <img src="{{Config::get('app.static_image_product_url').$imageObj->image_name}}"/ style="width:100%">
                                <input type="hidden" value="{{$imageObj->image_name}}" name="name_image[]" />
                                @if($imageObj->image_name == $productObj->image)
                                    <input type="radio" value="{{$imageObj->image_name}}" name="set_defualt_image" checked>Chọn ảnh đại diện
                                @else
                                    <input type="radio" value="{{$imageObj->image_name}}" name="set_defualt_image">Chọn ảnh đại diện
                                @endif
